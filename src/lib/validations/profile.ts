...
export const userThemeSchema = z.object({
  ...
  banner_style: z.enum(["glass", "dimensional", "minimal", "led", "gradient"]),
  banner_height: z.number().min(200).max(500),
  banner_fade_start: z.number().min(0).max(80),
  banner_fade_intensity: z.enum(["subtle", "medium", "strong"]),
