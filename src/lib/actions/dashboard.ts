...
import { linkSchema, profileSchema, reorderLinksSchema } from "@/lib/validations/profile";
...
export async function updateProfileAction(input: unknown): Promise<ActionState> {
  ...
  const { error: userError } = await supabase
    .from("users")
    .upsert({ id: userId, email, username, avatar_url: avatarUrl || null, active: true });
...
export async function upsertLinkAction(input: unknown): Promise<ActionState> {
...
export async function reorderLinksAction(input: unknown): Promise<ActionState> {
  ...
  const updates = parsed.data.ids.map((id, index) =>
    supabase.from("links").update({ order_position: index }).eq("id", id).eq("user_id", userId),
  );
