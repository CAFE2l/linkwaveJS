        ...
        >
          <article
            ...
            style={{
              ...
              backdropFilter: "blur(min(var(--ut-card-blur, 14px), 18px)) saturate(165%)",
              WebkitBackdropFilter:
                "blur(min(var(--ut-card-blur, 14px), 18px)) saturate(165%)",
              borderColor: "var(--ut-card-glass-border, rgba(255,255,255,0.45))",
              boxShadow:
          ...
          >
            ...
            <div className={`relative text-center ${isPreview ? "-mt-10 px-2" : "-mt-12 px-3"}`}>
              <ProfileAvatar
              ...
              <h1
                className={`font-black leading-tight tracking-tight drop-shadow-sm ${
                  isPreview ? "mt-2.5 text-xl" : "mt-3 text-2xl sm:text-[1.75rem]"
                }`}
              >
                {user.name || user.username}
              </h1>
              <p
                className={`${isPreview ? "mt-0.5 text-xs" : "mt-1 text-sm"} font-bold`}
              ...
              {bio && (
                <p
                  className={`mx-auto max-w-sm leading-relaxed ${
                    isPreview ? "mt-2 line-clamp-2 text-xs" : "mt-2.5 text-sm"
                  }`}
                  style={{ color: "var(--ut-text-secondary)" }}
                >
                  {bio}
            ...
            <div
              ...
              style={{ background: "var(--ut-card-glass-border, rgba(255,255,255,0.25))" }}
            />

            <div
              className={`${isPreview ? "space-y-2 px-1 pb-1" : "space-y-3 px-2 pb-2"} ${
            ...
            >
              {isPreview ? (
                ...
                <AnimatedLinks
                  transitionEffect={theme.enable_animations ? theme.transition_effect : "none"}
                >
                  {linksContent}
                </AnimatedLinks>
              )}
          ...
          <div className={`${isPreview ? "mt-2.5" : "mt-4"} text-center`}>
            <NextLink
              href="/"
              tabIndex={isPreview ? -1 : undefined}
              className={`inline-flex items-center gap-1.5 rounded-full border border-white/40 bg-white/15 font-black backdrop-blur-md transition hover:-translate-y-0.5 hover:bg-white/25 ${
                isPreview
              ...
              style={{ color: "var(--ut-text-secondary)" }}
            >
              <Image src="/brand/icon.png" alt="" width={15} height={15} />
              LinkWave
            </NextLink>
          </div>
        </main>
